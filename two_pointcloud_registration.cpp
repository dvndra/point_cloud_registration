...
// Hyper parameters
#define LEAF_SIZE .1
#define normal_radius 0.8
#define feature_radius .5
#define RANSAC_Inlier_Threshold 5.5
#define RANSAC_Iterations 5000
#define CorrRejDist_Maximum_Distance 5
#define ICP_Max_Correspondence_Distance 1.5
...
int main(int argc, char** argv)  {		
	...
	string src_file = "/Users/devendraswami/Downloads/USC/summer_19/PCL_clean/Plate_no_change_500000_scaled.pcd";
	string tgt_file = "/Users/devendraswami/Downloads/USC/summer_19/PCL_clean/Plate_change_500000.pcd";
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr src_filtered(new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::VoxelGrid<pcl::PointXYZRGB> sor;
	sor.setInputCloud(src_original);
	sor.setLeafSize(LEAF_SIZE, LEAF_SIZE, LEAF_SIZE);
	sor.filter(*src_filtered);

	pcl::PointCloud<pcl::PointXYZRGB>::Ptr tgt_filtered(new pcl::PointCloud<pcl::PointXYZRGB>);
	sor.setInputCloud(tgt_original);
	sor.setLeafSize(LEAF_SIZE, LEAF_SIZE, LEAF_SIZE);
	sor.filter(*tgt_filtered);

	cerr << "Src PointCloud after filtering: " << src_filtered->width * src_filtered->height
		<< " data points (" << pcl::getFieldsList(*src_filtered) << ")." << endl;

	cerr << "Tgt PointCloud after filtering: " << tgt_filtered->width * tgt_filtered->height
		<< " data points (" << pcl::getFieldsList(*tgt_filtered) << ")." << endl;
	...
	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> keypoints_src_color_handler(keypoints_src_visualize_temp, 0, 255, 0);
	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> keypoints_tgt_color_handler(keypoints_tgt_visualize_temp, 0, 255, 0);
	...
	for (int i = 0; i < good_correspondences->size(); ++i)
	{
		...
		//corresp_viewer.addLine<pcl::PointXYZ, pcl::PointXYZ>(keypoints_src_visualize_temp->points[i], keypoints_src_visualize_temp->points[i+10], r, g, b, lineID);
		//corresp_viewer.addLine<pcl::PointXYZ, pcl::PointXYZ>(keypoints_tgt_visualize_temp->points[i], keypoints_tgt_visualize_temp->points[i+10], r, g, b, lineID2);
		//break;
	}
	...
	icp_alignment_viewer.addPointCloud<pcl::PointXYZRGB>(src_initial_output, src_initial_color_handler, "ICP Aligned Src cloud");
